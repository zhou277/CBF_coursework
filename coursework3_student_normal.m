...
penalty_p = 200;           % Slack variable penalty in the QP
gamma1 = 3;               % CLF decay rate for orientation
k_1 = 15;                 % Tuning parameter k1 (affects the safety constraint)
...
for k = 1:n_steps
    ...
    dotB = 2 * dx * v * cos(theta) + 2 * dy * v * sin(theta);  % Ḃ(x)
    psi0 = B;
    psi0_dot = dotB;
    psi1 = psi0_dot + k_1 * psi0;
    ...
    % Lf²B: terms independent of a and omega
    Lf2B = 2 * v^2;
    
    % Final CBF constraint in the form A_cbf * [omega; a; delta] ≤ b_cbf
    A_cbf = [LgB_omega, LgB_a, -1];
    b_cbf = -Lf2B - k_1 * psi0_dot - k_2 * psi1;    
    ...
    % CLF 定义：使用 atan(dgy/dgx)
    desired_theta = atan(dgy/dgx);  % 直接用 atan 得到期望角度
    ...
    % 构造 V̇1 + γ₁V₁ ≤ δ 的线性不等式项
    A_clf1 = [2 * e_theta, -2 * lambda * (dgx * cos(theta) + dgy * sin(theta)), -1];
    b_clf1 = -gamma1 * V_1;
    
    % 构造 V̇2 + γ₂V₂ ≤ δ 的线性不等式项
